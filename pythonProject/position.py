...
class Server:
    ...
    def johnny_control(self): # rate controller
        ...
        for subName in self.subjectNames:
            ...
            # position controller
            # global origin = [-351,685,76] (x,y,z) in mm
            ...
            if count%2==0:
                ...
                P_des = np.array([-35.1, 68.5, 7.6])  # desired position in cm
            else:
                P_des = np.array([85.5, 59.2, 7.6])  # desired position in cm
            #P_des = np.array([85.5, 59.2, 7.6])  # desired position in cm
            #print(T[-1])

            P_des = np.array([-35.1, 68.5, 7.6])  # desired position in cm
            P_des = np.array([23.8, 58.8, 7.9])  # desired position in cm
            P=pos[0:2]/1000 #in m
            obs_center=np.array([234,585])/1000 # in m
            obs_radi = 0.05 # in m

            rho0 = 0.15 # in m
            rho=(LA.norm(P-obs_center)-obs_radi) # in m
            #rho = (LA.norm(P - obs_center)) / 10
            Krep = 500
            if rho<=rho0:
                v_rep = Krep*(1/rho - 1/rho0)/pow(rho, 3)*(obs_center-P)
            else:
                v_rep = 0
            v_rep = 0
            ...
            v_des = v_des-v_rep
            ref_vel[0] = 1*np.array([np.cos(rot[2]) , np.sin(rot[2])])@v_des.transpose()
            if ref_vel[0] >= 10:
                ref_vel[0] = 10
            elif ref_vel[0] <= 0:
                ref_vel[0] = 0
            #ref_vel[0]=0
            ref_vrot[2] = 0.5 * math.atan2(np.array([-np.sin(rot[2]),np.cos(rot[2])])@v_des.transpose() , np.array([np.cos(rot[2]),np.sin(rot[2])])@v_des.transpose())/(np.pi/2)
            ref_vrot[2] = 180/np.pi*ref_vrot[2]
            if ref_vrot[2] >= 20:
                ref_vrot[2] = 20
            elif ref_vrot[2] <= -20:
                ref_vrot[2] = -20
            #ref_vrot[2] = -0
            #for i=1:5
            #    theta_dot(i, 1) = 4 * atan2([-sin(theta_cart(i)) cos(theta_cart(i))] * v(i,:)', ...
            #    [cos(theta_cart(i))
            #    sin(theta_cart(i))]*v(i,:)')/(pi/2);
            #end
            print('Linear_velocity' + str(ref_vel[0])+' Angular velocity: ' + str(ref_vrot[2]))
            ...
            # enter the desired angular and translational velocities (deg/s, cm/s)
