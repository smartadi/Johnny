...
class Server:
    ...
    def johnny_control(self): # rate controller
        ...
        for subName in self.subjectNames:
            ...
            ref_vrot_comm = 0.4 * math.atan2(np.array([-np.sin(rot[2]),np.cos(rot[2])])@v_des.transpose() , np.array([np.cos(rot[2]),np.sin(rot[2])])@v_des.transpose())/(np.pi/2)
            ...
            self.ref['Johnny08'][0] = ref_vel_comm
            #self.ref['Johnny08'][1] = 0
            #self.ref['Johnny08'][0] = 0
            ref_vrot = self.ref[subName][1]
            ref_vel = self.ref[subName][0]
