...
class Server:
    ...
    def johnny_update(self):
        ...
        for subName in self.subjectNames:
            ...
            data_rz = int(np.array([[0, 1, 0]]) @ vb.T* 255/1000)

            data_v = int(np.linalg.norm(data_v) * 255 / 100)

            print("v sent")
            print(data_v)
            print(data_rz)

            data_rz = data_rz + int((10 * 180 * (1 / np.pi) * (ref_vrot[2] - rot[2]))) + 1800

            self.data.update({subName: [data_v, data_rz]})

            self.plotterv = np.append(self.plotterv, [vb], axis=0)
...
if __name__ == '__main__':
    ...
    for j in range(500):
        ...
        for name in Robot.subjectNames:
            ...
            p[0,3*i:3*i+3] = Robot.get_estimate()[name][0]
            v[0,3*i:3*i+3] = Robot.get_estimate()[name][2]

            r[0, 3 * i:3 * i + 3] = Robot.get_estimate()[name][1]
            a[0, 3 * i:3 * i + 3] = Robot.get_estimate()[name][3]
